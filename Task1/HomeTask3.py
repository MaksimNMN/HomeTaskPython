...
A_entered = input("Введите элементы списка раздельно: ").split()
A_num = list(map(int, A_entered))
if len(A_num) != N:
    print('Введенные элементы не соответствуют данному количеству!')
else:
...
N = abs(int(input('Введите элементы списка А: ')))
A_entered = input("Введите элементы списка раздельно: ").split()
A_num = list(map(int, A_entered))
if len(A_num) != N or N == 0:
    ...
    for i in range(1, N):
        ...
        if count < min:
            ...
            index = i
    print(
        f'Число {A_num[index]} в  списке A ближе по величине к числу {X}, их разница  {abs(X - A_num[index])}')
